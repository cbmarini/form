...
\noindent See also startpadic (\ref{prestartpadic}) and chapter~\ref{floatingpoint} on floating point and p-adic arithmetic.
...
\noindent \FORM{} also accepts:
\begin{verbatim}
#startpadic <p>,<N>
\end{verbatim}
The parameter \texttt{<p>} must be prime and \texttt{<N>} must be an integer.
Negative values are allowed and select an absolute precision below $O(p^0)$;
zero selects precision $O(p^0)$.

\noindent See also endpadic (\ref{preendpadic}) and chapter~\ref{floatingpoint}
on floating point and p-adic arithmetic.

\noindent \#startpadic and \#startfloat are mutually exclusive in one run.
