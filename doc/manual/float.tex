...
\end{description}
The floating point and p-adic systems are mutually exclusive. If p-adics are
active, \texttt{\#StartFloat} gives an error. Likewise, if floats are active,
\texttt{\#StartPadic} gives an error.
...
\section{P-adic coefficients}
\FORM{} can also work with p-adic coefficients. This functionality uses FLINT
and follows the same activation model as floating point numbers.
\begin{description}
\item[\#StartPadic] Initializes the p-adic system:
\begin{verbatim}
    #StartPadic <p>, N=<N>
\end{verbatim}
or
\begin{verbatim}
    #StartPadic <p>,<N>
\end{verbatim}
Here \texttt{<p>} must be a prime and \texttt{<N>} must be an integer. Negative
values are allowed and select an absolute precision below $O(p^0)$; zero
selects precision $O(p^0)$.
The active context is global and arithmetic is truncated at $O(p^N)$.
\item[\#EndPadic] Releases all p-adic resources and disables p-adic arithmetic.
\item[ToPadic] Converts rational coefficients into p-adic coefficients in the
currently active context.
\end{description}
Like \texttt{ToFloat}, \texttt{ToPadic} operates at ground level, and can be
applied inside an \texttt{Argument} environment for function arguments.
No numerical function-evaluation statement is provided for p-adics.
...
\item[Format padicprecision] This instruction controls p-adic printing. It can
be used as:
\begin{verbatim}
    Format padicprecision;
\end{verbatim}
for the full active precision $N$,
\begin{verbatim}
    Format padicprecision <n>;
\end{verbatim}
for output truncated to $\min(n,N)$ digits when the active precision is
positive, and
\begin{verbatim}
    Format padicprecision off;
\end{verbatim}
for the raw internal \texttt{padic\_(...)} representation.
\end{description}
